...
export function initImageTool({ tempCanvasEl, tempCtxEl, mainCtxEl }) {
...
function imagePointerMoveOLD(e, canvas) {
    if (!activeImage) return false;
    ...
    if (activeImage.mode == null) {
        ...
        activeImage.x = pos.x - activeImage.offsetX;
        activeImage.y = pos.y - activeImage.offsetY;
    } else if (activeImage.mode === "resize-prop") {
        const v = toLocalPoint(pos.x, pos.y);
        const s = activeImage.startState;

        const dx = v.x - s.local.x;
        const dy = v.y - s.local.y;

        let newW = s.width + dx;
        let newH = newW / s.aspect;

        activeImage.width = Math.max(20, newW);
        activeImage.height = Math.max(20, newH);
    } else if (activeImage.mode === "resize-hor") {
        const v = toLocalPoint(pos.x, pos.y);
        const s = activeImage.startState;

        activeImage.width = Math.max(20, s.width + (v.x - s.local.x));
    } else if (activeImage.mode === "resize-ver") {
        const v = toLocalPoint(pos.x, pos.y);
        const s = activeImage.startState;

        activeImage.height = Math.max(20, s.height + (v.y - s.local.y));
    } else if (activeImage.mode === "rotate") {
        const c = center();
        canvas.style.cursor = "grabbing";
        activeImage.rotation =
